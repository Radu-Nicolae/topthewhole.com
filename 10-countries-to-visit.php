<!DOCTYPE html>
<html class="no-js" lang="zxx">
   <head>
      <meta charset="utf-8" />
      <meta http-equiv="x-ua-compatible" content="ie=edge" />
      <title>Top 10 | Countries To Visit</title>
      <meta name="keywords" content="Top Countries to visit, Top 10 countries to visit, countries to visit, 10 countries to visit, best countries to visit, top 10 best countries to visit, visit spain, visit new zeeland, visit turkey, visit romania, visit colombia, visit malaysia, visit peru, visit czech republic, visit england, visit ireland">
      <meta name="viewport" content="width=device-width, initial-scale=1" />
      <link rel="icon" href="img/small-logo.png" type="image/png">
      <link rel="manifest" href="site.webmanifest">
      <link rel="stylesheet" href="css/bootstrap.min.css" />
      <link rel="stylesheet" href="css/owl.carousel.min.css" />
      <link rel="stylesheet" href="css/magnific-popup.css" />
      <link rel="stylesheet" href="css/font-awesome.min.css" />
      <link rel="stylesheet" href="css/themify-icons.css" />
      <link rel="stylesheet" href="css/nice-select.css" />
      <link rel="stylesheet" href="css/flaticon.css" />
      <link rel="stylesheet" href="css/gijgo.css" />
      <link rel="stylesheet" href="css/animate.min.css" />
      <link rel="stylesheet" href="css/slick.css" />
      <link rel="stylesheet" href="css/slicknav.css" />
      <link rel="stylesheet" href="css/style.css" />
      <link rel="stylesheet" href="css/responsive.css">
      <link rel="stylesheet" href="css/more.css">
   
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-LVGKPM32XB"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-LVGKPM32XB');
</script>
   </head>
   <body>
      
      <?php
         include "components/header.php"
      ?>  
  
      <!-- bradcam_area  -->
      <div class="bradcam_area bradcam_bg_1 color">
       <div class="container">
           <div class="row">
               <div class="col-xl-12">
                   <div class="bradcam_text text-center">
                       <h3 style="color:white; font-size: 350%;">Top 10</h3>
                   </div>
               </div>
           </div>
       </div>
   </div>
      <!-- /bradcam_area  -->
 
    <!--================Blog Area =================-->
    <!-- <section>
      <div class="join-us-container" style="margin-bottom: 0px">
          <p class="text-join-us">Revista ÇA VA <br> recrutează!</p>
          <br>
          <a href="https://docs.google.com/forms/d/e/1FAIpQLSeJsJWOekLQbKcJnYrO-OGYbqcBpVx5kQ7fOgRWH6h822cy4g/viewform?usp=sf_link" target="_blank"><button type="button" class="btn btn-primary btn-lg join-us-button">Learn More</button></a>
      </div>
    </section> -->

    <section>
       <div class="single-blog-container">
         <div class="article-container">
            
            <header class="entry-header">
               <h1 class="entry-title itemprop">Countries you Must Visit</h1>
               <div class="entry-meta sr-only">
                  <meta itemprop="datePublished" content="2021-02-16">
                  <meta itemprop="dateModified" content="2021-02-16">
                  <span class="byline">  <span class="author vcard"><a class="url fn n" href="https://luanaionica.com/"></a></span> </span>
                  <meta itemprop="author" content="Luana Ionica">
                  <link id="page-url" itemprop="mainEntityOfPage" href="https://topthewhole.com/10-countries-to-visit.php">
                  <a href="https://topthewhole.com/10-countries-to-visit.php" rel="category tag"></a>
               </div>
               <!-- .entry-meta --> <br>
            </header>


            <style>
               .bigger-code{
                  font-size: 120%;
               }
               h2{
                  font-size: 190% !important;
               }
            </style>
            <p>
                <img 
                   src="img/articles/10-countries/image1.jpg" 
                   class="image-article-blog" 
                   alt="Top 10 Countries To Visit"
                   title="Top 10 Countries To Visit"
                >
             </p>

            <div class="text-container-article-blog">
               
               <p class="bigger-code">
If you love travelling, you are in the right place, reading the perfect article for you. Whether you are looking for a place to spend your holiday or a weekend getaway, this is the top 10 Countries you have to visit!               </p>

               <!-- SPAIN -->
               <div>
                  <h1 class="entry-title itemprop">
                     10. SPAIN         
                  </h1>
   
                  <p class="bigger-code">
                     There are a lot of things you can do and see in Spain, as it is such a beautiful country. Here are a few of those things!
                  </p>
   
                  <h2>
                     1.	Barcelona’s Sagrada Familia   
                  </h2>
   
                  <p class="bigger-code">
                     This is one of the most well-known attractions in Spain, and one of the most beautiful as well. Even though its construction began in the 1800s, the impressive church is still unfinished, but that does not make it less of an architectural masterpiece.
                  </p>
                  
                  <p>
                     <img 
                        src="img/articles/10-countries/image2.jpg" 
                        class="image-article-blog" 
                        alt="Sagrada Familia"
                        title="Sagrada Familia"
                     >
                  </p>
                  
                 
   
                 <h2>
                  2.	The Prado and Paseo del Artes, Madrid
                 </h2>
   
                 <p class="bigger-code">
                  The Prado is one of the world’s biggest art museums, with one of the richest collections. It's no wonder this is known as El Paseo del Arte, the Boulevard of the Arts.
                 </p>
   
                 <p>
                  <img 
                     src="img/articles/10-countries/image3.jpg" 
                     class="image-article-blog" 
                     alt="The Prado and Paseo del Artes"
                     title="The Prado and Paseo del Artes"
                  >
               </p>
   
   
               <h2>
                  3.	Seville Cathedral and Alcazar
               </h2>
   
               <p class="bigger-code">
                  Seville Cathedral is a Roman Catholic Cathedral, and in the 16<sup>th</sup> century, when it has been built, it was the largest cathedral in the world (11,520 square meters). Even now, it is one of the most beautiful buildings in the world. 
               </p>
                 
               <p>
                  <img 
                     src="img/articles/10-countries/image4.jpg" 
                     class="image-article-blog" 
                     alt="Seville Cathedral and Alcazar"
                     title="Seville Cathedral and Alcazar"
                  >
               </p>
   
   
               <h2>
                  4.	Plaza Mayor, Madrid
               </h2>
   
               <p class="bigger-code">
                  Plaza Mayor is another place that you must visit. It dates back to the 15<sup>th</sup> century, when it was used as a market. Today it is there to impress un with its amazing architecture and history.
               </p>
   
               <p>
                  <img 
                     src="img/articles/10-countries/image5.jpg" 
                     class="image-article-blog" 
                     alt="Plaza Mayor"
                     title="Plaza Mayor"
                  >
               </p>
   
               <h2>
                  5.	Toledo's Old City
               </h2>
   
               <p class="bigger-code">
                  Toledo is known as the “Imperial City” and it sure looks like it. The mystical fortress looks like one of the Disney castles brought to life. It is a city with a rich history and culture as it dates back to 500 AD. 
               </p>
   
               <p>
                  <img 
                     src="img/articles/10-countries/image6.jpg" 
                     class="image-article-blog" 
                     alt="Toledo's Old City"
                     title="Toledo's Old City"
                  >
               </p>
   
               </div>
               
               <!-- NEW ZEELAND -->
               <div>
                  <h1 class="entry-title itemprop">
                     9. New Zeeland
                  </h1>

                  <p class="bigger-code">
                     If you are a nature lover, this is the country for you. In New Zealand you can find anything from untamed wilderness to rich culture. This is a country of adventure and if this is your scene then the list of activities below will convince you to book a flight right now!
                  </p>

                  <h2>
                     - Mountain Hut
                  </h2>

                  <p class="bigger-code">
                     Spend an unforgettable night in one of the many mountain huts. There are more than 1000 such isolated places in New Zealand and it is an experience you must try at least once in your life. The views are breath taking, and the solitude is the reprieve we all need from time to time.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image7.jpg" 
                        class="image-article-blog" 
                        alt="Mountain Hut"
                        title="Mountain Hut"
                     >
                  </p>


                  <h2>
                     - Swim with dolphins
                  </h2>

                  <p class="bigger-code">
                     If you have ever dreamed of such a thing, New Zealand is the place to try it out. On the South Island you can find a smaller species of dolphins, the Dusky Dolphins. Being that small (maximum 2 meters) and very peaceful, they make the perfect scuba diving partner. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image8.jpg" 
                        class="image-article-blog" 
                        alt="Swim with dolphins"
                        title="Swim with dolphins"
                     >
                  </p>

                  <h2>
                     - Bungy Jumping
                  </h2>

                  <p class="bigger-code">
                     We have all heard of this extreme activity before, some of us may have even tried it out, but the views in New Zealand make it the perfect place for such an experience. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image9.jpg" 
                        class="image-article-blog" 
                        alt="Bungy Jumping"
                        title="Bungy Jumping"
                     >
                  </p>


                  <h2>
                     - Milford Sound
                  </h2>

                  <p class="bigger-code">
                     Milford Sound is the most visited touristic attraction in New Zealand, and it is often referred to as the 8th Wonder of the World. You can get there either by Kayak or a Boat Cruise. Both experiences are unforgettable and a must for all the people who enjoy travelling. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image10.jpg" 
                        class="image-article-blog" 
                        alt="Milford Sound"
                        title="Milford Sound"
                     >
                  </p>



                  <h2>
                     - Tangariro Crossing
                  </h2>

                  <p class="bigger-code">
                     Tangariro Crossing is one of the best hikes in New Zealand. The views on this trek are truly epic! 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image11.jpg" 
                        class="image-article-blog" 
                        alt="Tangariro Crossing"
                        title="Tangariro Crossing"
                     >
                  </p>

               </div>

               <!-- TURKEY -->
               <div>
                  <h1 class="entry-title itemprop">
                     8. Turkey
                  </h1>

                  <p class="bigger-code">
                     Turkey has an incredibly rich culture and is home to numerous locations that are beyond your dreams. It is full of ancient sites, unparalleled destinations, and unique beauty.
                  </p>

                  <h2>
                     - Aya Sophia
                  </h2>

                  <p class="bigger-code">
                     Aya Sophia has been renowned as one of the most beautiful buildings in the world. The monument is certainly a must do for every person visiting this country. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image12.jpg" 
                        class="image-article-blog" 
                        alt="Aya Sophia"
                        title="Aya Sophia"
                     >
                  </p>


                  <h2>
                     - Ephesus 
                  </h2>

                  <p class="bigger-code">
                     The mighty ruin of Ephesus is not to be missed. This is one of the only still-standing Roman cities. You can spend almost a day exploring the sites and seeing how the romans lived.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image13.jpg" 
                        class="image-article-blog" 
                        alt="Ephesus"
                        title="Ephesus"
                     >
                  </p>

                  
                  <h2>
                     - Pamukkale
                  </h2>

                  <p class="bigger-code">
                     Pamukkale, or Cotton Castle in English, is one of the most well-known natural wanders of Turkey. For the best photographs you should come at dusk and be amazed by how the travertines glow as the sun sinks.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image14.jpg" 
                        class="image-article-blog" 
                        alt="Pamukkale"
                        title="Pamukkale"
                     >
                  </p>


                  <h2>
                     - Aspendos
                  </h2>

                  <p class="bigger-code">
                     The jaw-dropping monument used to be a Roman theatre. It dates back from the classical age and is considered to be the finest surviving example. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image15.jpg" 
                        class="image-article-blog" 
                        alt="Aspendos"
                        title="Aspendos"
                     >
                  </p>


                  <h2>
                     - Pergamum
                  </h2>

                  <p class="bigger-code">
                     Once home to the most ancient library in the world, now Pergamum in a must-see attraction if you decide to visit Turkey. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image16.jpg" 
                        class="image-article-blog" 
                        alt="Pergamum"
                        title="Pergamum"
                     >
                  </p>

               </div>

               <!-- ROMANIA -->
               <div>
                  <h1 class="entry-title itemprop">7. Romania</h1>

                  <p class="bigger-code">
                     Romania is not such a well-known country, but it is definitely the place to visit if you are looking for more rustic experiences.
                  </p>

                  <h2>
                     - Bran Castle
                  </h2>

                  <p class="bigger-code">
                     Because of the Dracula stories that have travelled all over the world, the exquisite fortress is one of the most popular attractions in Romania. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image17.jpg" 
                        class="image-article-blog" 
                        alt="Bran Castle"
                        title="Bran Castle"
                     >
                  </p>


                  <h2>
                     - Peles Castle
                  </h2>

                  <p class="bigger-code">
                     Built in the 19<sup>th</sup> century as a summer residence for the royal family, Peles Castle is one of the most beautiful castles you will ever see. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image18.jpg" 
                        class="image-article-blog" 
                        alt="Peles Castle"
                        title="Peles Castle"
                     >
                  </p>


                  <h2>
                     - Bucovina
                  </h2>

                  <p class="bigger-code">
                     Bucovina’s Monasteries are Romania’s most important religious attraction. Their unique and vivid paintings living permanently in a person’s memory.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image19.jpg" 
                        class="image-article-blog" 
                        alt="Bucovina"
                        title="Bucovina"
                     >
                  </p>



                  <h2>
                     - The Palace of Parliament
                  </h2>

                  <p class="bigger-code">
                     The Palace of Parliament in Bucharest is Europe’s biggest building and a sight you cannot miss. Also, it is the heaviest building, as well as the largest legislature building worldwide.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image20.jpg" 
                        class="image-article-blog" 
                        alt="The Palace of Parliament"
                        title="The Palace of Parliament"
                     >
                  </p>



                  
                  <h2>
                     - The Sphinx
                  </h2>

                  <p class="bigger-code">
                     The mysterious rock formation, high in the Bucegi Mountains, is one of the most interesting natural things you will ever see. Legend claims that the rock represents a long-forgotten god.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image21.jpg" 
                        class="image-article-blog" 
                        alt="The Sphinx"
                        title="The Sphinx"
                     >
                  </p>
               </div>

               <!-- COLUMBIA -->
               <div>
                  <h1 class="entry-title itemprop">6. Columbia</h1>
                  

                  <p class="bigger-code">
                     Columbia is a beautiful country with a lot of attractions. I would definitely recommend this country for a nature and adventure lover.
                  </p>

                  <h2>
                     - The lost City of Teyuna
                  </h2>

                  <p>
                     <img 
                        src="img/articles/10-countries/image22.jpg" 
                        class="image-article-blog" 
                        alt="The lost City of Teyuna"
                        title="The lost City of Teyuna"
                     >
                  </p>

                  <p class="bigger-code">
                     If you decide to go trekking in the lost City of Teyuna, you have to prepare for the 46km trek that will take you through jungles, hills and rivers.
                  </p>

                  

                  <h2>
                     - Totumo Volcano
                  </h2>

                  <p class="bigger-code">
                     Does mud-bathing in a 2,300m deep volcano sounds appealing to you? Columbia is the only place where you can have such an extraordinary experience.
                  </p>


                  <p>
                     <img 
                        src="img/articles/10-countries/image23.jpg" 
                        class="image-article-blog" 
                        alt="Totumo Volcano"
                        title="Totumo Volcano"
                     >
                  </p>
                  


                  <h2>
                     - Tayrona National Park
                  </h2>

                  <p class="bigger-code">
                     You can go hiking and swimming in Tatyrona National Park, a place full of nature and wanders. Just keep in mind that not all places are safe for swimming. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image24.jpg" 
                        class="image-article-blog" 
                        alt="Tayrona National Park"
                        title="Tayrona National Park"
                     >
                  </p>
                  


                  <h2>
                     - Cocora Valley
                  </h2>

                  <p class="bigger-code">
                     The Cocora Valley is the place that the towering 68m wax palms call home. You can go trekking through the cloud forest and enjoy the exquisite atmosphere.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image25.jpg" 
                        class="image-article-blog" 
                        alt="Cocora Valley"
                        title="Cocora Valley"
                     >
                  </p>

                  


                  <h2>
                     - The coffee country
                  </h2>

                  <p class="bigger-code">
                     You have the chance to visit the coffee factories and discover how exactly the coffee we love so much is made. Here you can even make your own coffee and bring it home as a holiday souvenir. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image255.jpg" 
                        class="image-article-blog" 
                        alt="The Coffee Country"
                        title="The Coffee Country"
                     >
                  </p>



               </div>

               <!-- MALAYSIA -->
               <div>
                  <h1 class="entry-title itemprop">5. Malaysia</h1>

                  <h2>
                     - Malacca
                  </h2>

                  <p class="bigger-code">
                     Malacca or Melaka is a historic state full of culture. Also known as the state with the most interesting architecture in Malaysia, Malacca is filled with museums, galleries, and places of historic interest.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image27.jpg" 
                        class="image-article-blog" 
                        alt="Malacca"
                        title="Malacca"
                     >
                  </p>

                  <h2>
                     - Taman Negara National Park
                  </h2>

                  <p class="bigger-code">
                     Located in the state of Pahang, Taman Negara National Park is one of the best places to go if you enjoy trekking. It has everything from 100km road trips to gentle hikes for beginners. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image28.jpg" 
                        class="image-article-blog" 
                        alt="Taman Negara National Park"
                        title="Taman Negara National Park"
                     >
                  </p>

                  <h2>
                     - Kota Bharu
                  </h2>

                  <p class="bigger-code">
                     Many people fail to visit this city because of its more conservative reputations, but here you can find a wide range of mosques and other religious sites, as well as the old royal palaces which are still the home of the current Sultan of Kelantan. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image29.jpg" 
                        class="image-article-blog" 
                        alt="Kota Bharu"
                        title="Kota Bharu"
                     >
                  </p>

                  <h2>
                     - Sepilok Orangutan Rehabilitation Centre in Sabah
                  </h2>

                  <p class="bigger-code">
                     The centre helps orangutans that have been taken as pets or left to tend for themselves in the wilderness adapt to their life. Once they are ready, they are released. You can watch the extraordinary animals and even interact with them.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image30.jpg" 
                        class="image-article-blog" 
                        alt="Sepilok Orangutan Rehabilitation Centre in Sabah"
                        title="Sepilok Orangutan Rehabilitation Centre in Sabah"
                     >
                  </p>

                  <h2>
                     - Perhentian Islands
                  </h2>

                  <p class="bigger-code">
                     Very close to the cost of Thailand, the islands are known for their beaches and incredibly clear water. Scuba diving remains the favourite activity in these parts.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image31.jpg" 
                        class="image-article-blog" 
                        alt="Perhentian Islands"
                        title="Perhentian Islands"
                     >
                  </p>



               </div>

               <!-- PERU -->
               <div>
                  <h1 class="entry-title itemprop">4. Peru</h1>

                  <p class="bigger-code">
                     Rainbow mountains, rare wildlife and lost cities make Peru a place you cannot miss in your travels. Here are some of the things that await you there.
                  </p>

                  <h2>
                     - Machu Picchu
                  </h2>

                  <p class="bigger-code">
                     An unforgettable 4-day trek will take you to the secret city. You will not regret this, but you need to be aware that the numbers are limited. Even if you are too late to book a place, there are a lot of other options for you to try out.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image32.jpg" 
                        class="image-article-blog" 
                        alt="Machu Picchu"
                        title="Machu Picchu"
                     >
                  </p>

                  <h2>
                     - Cusco
                  </h2>

                  <p class="bigger-code">
                     You will need to spend a day or two here, as the city is home to 15 historic sites as well as San Blas neighbourhood, an artisan quarter filled with galleries and boutiques. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image33.jpg" 
                        class="image-article-blog" 
                        alt="Cusco"
                        title="Cusco"
                     >
                  </p>


                  <h2>
                     - Ballestas Islands
                  </h2>

                  <p class="bigger-code">
                     The islands are any nature lover’s dream. They are teeming with colonies of sea lions, penguins and seabirds including albatrosses, boobies, and pelicans. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image34.jpg" 
                        class="image-article-blog" 
                        alt="Ballestas Islands"
                        title="Ballestas Islands"
                     >
                  </p>


                  <h2>
                     - Lake Titicaca
                  </h2>

                  <p class="bigger-code">
                     The lake, known as the birthplace of the sun, is the highest navigable lake in the world. Here you can see the man-made islands and the floating homes of the Uros Indians. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image35.jpg" 
                        class="image-article-blog" 
                        alt="Lake Titicaca"
                        title="Lake Titicaca"
                     >
                  </p>



                  <h2>
                     - The Amazon
                  </h2>

                  <p class="bigger-code">
                     Peru is where the Amazon starts and you can enjoy the wildlife of it. Manu National Park, one of the parks here, is one of the most bio-diverse areas in the whole world, boasting at least 1,000 birds and over 200 mammal species. You cannot miss it!
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image36.jpg" 
                        class="image-article-blog" 
                        alt="The Amazon"
                        title="The Amazon"
                     >
                  </p>






               </div>

               <!-- CZECH REPUBLIC -->
               <div>
                  <h1 class="entry-title itemprop">3. Czech Republic</h1>

                  <p class="bigger-code">
                     Even though Czech Republic is one of the smallest nations, it will not disappoint travellers looking to visit central Europe. Considering its size, it is an easy country to get around and it has some breath-taking attractions. 
                  </p>


                  <h2>
                     - Prague Castle
                  </h2>
                  
                  <p class="bigger-code">
                     Prague is one of the most well-known places to visit in Czech Republic, and for a reason. The Prague Castle is just one of the many examples of what the city has to offer. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image37.jpg" 
                        class="image-article-blog" 
                        alt="Prague Castle"
                        title="Prague Castle"
                     >
                  </p>


                  <h2>
                     - Prague Charles Bridge
                  </h2>

                  <p class="bigger-code">
                     The 520-meter span structure over the Vltava River was built in 1357 and has numerous attractions besides its unique architecture, including a considerable number of statues. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image38.jpg" 
                        class="image-article-blog" 
                        alt="Prague Charles Bridge"
                        title="Prague Charles Bridge"
                     >
                  </p>


                  <h2>
                     - St. Vitus Cathedral 
                  </h2>

                  <p class="bigger-code">
                     The Cathedral has a long history, as it dates back to 925. It is best known as the home of the Bohemian Coronation Jewels and it consists in a place you must see if you decide to visit the Czech Republic.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image39.jpg" 
                        class="image-article-blog" 
                        alt="St. Vitus Cathedral"
                        title="St. Vitus Cathedral"
                     >
                  </p>


                  <h2>
                     - The Clementinum and Strahov Monastery
                  </h2>

                  <p class="bigger-code">
                     These spectacular libraries can be found in Prague, just next to the Charles Bridge. I will just let the pictures speak on their behalf, as no words could ever do them justice.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image40.jpg" 
                        class="image-article-blog" 
                        alt="The Clementinum and Strahov Monastery"
                        title="The Clementinum and Strahov Monastery"
                     >
                  </p>


                  <h2>
                     - The Bohemian Paradise
                  </h2>

                  <p class="bigger-code">
                     This area is known for its outstanding natural beauty and many massive rock formations that come out of the ground like spikes and pillars, formed during tens of thousands of years by erosion and by the elements.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image41.jpg" 
                        class="image-article-blog" 
                        alt="The Bohemian Paradise"
                        title="The Bohemian Paradise"
                     >
                  </p>



               </div>

               <!-- ENGLAND -->
               <div>
                  <h1 class="entry-title itemprop">2. England</h1>

                  <p class="bigger-code">
                     England is a country with a rich history. It is filled with castles and all kinds of attractions you might want to see. This is certainly a place you want to visit at least once in your life.
                  </p>

                  <h2>
                     - Chinatown
                  </h2>

                  <p class="bigger-code">
                     At the east end of London, you can find Chinatown. The large inflow of emigrants from Hong Kong made it possible for you to enjoy some of the best Chinese restaurants right in London.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image42.jpg" 
                        class="image-article-blog" 
                        alt="Chinatown"
                        title="Chinatown"
                     >
                  </p>
                  

                  <h2>
                     - Cambridge University
                  </h2>

                  <p class="bigger-code">
                     Founded in 1209, Cambridge University is one of the oldest universities in the world. These days, the university has 18,000 students, in 31 colleges.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image43.jpg" 
                        class="image-article-blog" 
                        alt="Cambridge University"
                        title="Cambridge University"
                     >
                  </p>


                  <h2>
                     - Hadrian’s Wall
                  </h2>

                  <p class="bigger-code">
                     Stretching from Ravenglass to Wallsend, Hadrian’s Wall was a fortification built by the Romans from 122 AD onwards. Today, visitors can still see a big part of the wall, as it was restructured in the 19<sup>th</sup> century.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image44.jpg" 
                        class="image-article-blog" 
                        alt="Hadrian’s Wall"
                        title="Hadrian’s Wall"
                     >
                  </p>



                  <h2>
                     - Durham Castle
                  </h2>

                  <p class="bigger-code">
                     A strong point of King Norman, the castle was built in the 11th century and served as a display of power and prestige. The castle in now occupied by the Durham University. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image45.jpg" 
                        class="image-article-blog" 
                        alt="Durham Castle"
                        title="Durham Castle"
                     >
                  </p>



                  <h2>
                     - York Minster
                  </h2>

                  <p class="bigger-code">
                     York Minster is on of the finest cathedrals in Great Britain and the largest one in Northern Europe. The cathedral was originally built in the 14th century. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image46.jpg" 
                        class="image-article-blog" 
                        alt="York Minster"
                        title="York Minster"
                     >
                  </p>
                  
               </div>

               <!-- IRELAND -->
               <div>

                  <h1 class="entry-title itemprop">1. Ireland</h1>

                  <p class="bigger-code">
                     Your traveller bucket list wouldn’t be complete without a visit in the beautiful country of Ireland!
                  </p>

                  <h2>
                     - The Ring of Kerry
                  </h2>

                  <p class="bigger-code">
                     The spectacular 111-mile-long tourist route is arguably Ireland’s most scenic route. It houses stunning islands to visit, many picturesque villages and wild sweeping mountains. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image47.jpg" 
                        class="image-article-blog" 
                        alt="The Ring of Kerry"
                        title="The Ring of Kerry"
                     >
                  </p>


                  <h2>
                     - The Rock of Cashel
                  </h2>

                  <p class="bigger-code">
                     The Rock of Cashel is one of Ireland’s most visited heritage sites. The magnificent group of medieval buildings includes structures even from the 12<sup>th</sup> century.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image48.jpg" 
                        class="image-article-blog" 
                        alt="The Rock of Cashel"
                        title="The Rock of Cashel"
                     >
                  </p>


                  <h2>
                     - The Dingle Peninsula
                  </h2>

                  <p class="bigger-code">
                     The Dingle Peninsula combines wild beauty, history and a glimpse of Irish culture and language. The region is designated as a Gaeltacht, where the Irish language and culture are protected by government subsidies.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image49.jpg" 
                        class="image-article-blog" 
                        alt="The Dingle Peninsula"
                        title="The Dingle Peninsula"
                     >
                  </p>



                  <h2>
                     - St. Stephan’s Green
                  </h2>

                  <p class="bigger-code">
                     St. Stephan’s Green is a great place to enjoy a day in nature, have a picnic and delight in everything nature has to offer. 
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image50.jpg" 
                        class="image-article-blog" 
                        alt="St. Stephan’s Green"
                        title="St. Stephan’s Green"
                     >
                  </p>
                  

                  <h2>
                     - Bunratty Castle and Folk Park
                  </h2>

                  <p class="bigger-code">
                     The Castle is the best-preserved medieval fortress in Ireland, dating back to 1425, while the Folk Park brings the life of 19th century ago Ireland to life.
                  </p>

                  <p>
                     <img 
                        src="img/articles/10-countries/image51.jpg" 
                        class="image-article-blog" 
                        alt="Bunratty Castle and Folk Park"
                        title="Bunratty Castle and Folk Park"
                     >
                  </p>

               </div>

               <br>

               <p class="bigger-code">
                  This was our top 10 countries you must visit! 10 more countries will come soon for even more inspiration for your travels. If you want to see more pictures you can access our <a href=" https://ro.pinterest.com/TopTheWhole" target="_blank" style="text-decoration: underline;">Pinterest account.</a> Have a great day!
               </p>


               </div>
            </div>
               
               <style>
                  .conclusions-container{
                     display: inline;
                  }


                  .text-conclusions-container{
                     display: flex;
                  }

                  @media(max-width:761px){
                     .text-conclusions-container{
                        display: block;
                     }
                  }

                  .text-conclusions{
                     padding-right: 20px;
                  }
                  @media(max-width:761px){
                     .txt-conclusions{
                        padding-right: 0px;
                     }
                  }

                  p{
                     line-height: 2;
                     font-size: 1rem;
                     margin-top: 0;
                     margin-bottom: 20px;
                     color: #363940 !important;
                     font-weight: 400;
                     font-size: 120%;
                  }

                  sup{
                     color: #363940;
                  }
                  
               </style>


            
            
               <?php
                  include "components/topPics.php";
               ?>


        </div>

    </section>
 

 
<!-- footer start -->
      <?php
         include "components/footer.php";
      ?>
      <!--/ footer end  -->
 
 
 <script src="https://cdn.jsdelivr.net/npm/darkmode-js@1.5.7/lib/darkmode-js.min.js"></script>
 <script>
    function addDarkmodeWidget() {
       new Darkmode().showWidget();
    }
    window.addEventListener('load', addDarkmodeWidget);
 </script>
 <script src="js/darkmode.js"></script>
 <script src="js/vendor/modernizr-3.5.0.min.js"></script>
 <script src="js/vendor/jquery-1.12.4.min.js"></script>
 <script src="js/popper.min.js"></script>
 <script src="js/bootstrap.min.js"></script>
 <script src="js/owl.carousel.min.js"></script>
 <script src="js/isotope.pkgd.min.js"></script>
 <script src="js/ajax-form.js"></script>
 <script src="js/waypoints.min.js"></script>
 <script src="js/jquery.counterup.min.js"></script>
 <script src="js/imagesloaded.pkgd.min.js"></script>
 <script src="js/scrollIt.js"></script>
 <script src="js/jquery.scrollUp.min.js"></script>
 <script src="js/wow.min.js"></script>
 <script src="js/nice-select.min.js"></script>
 <script src="js/jquery.slicknav.min.js"></script>
 <script src="js/jquery.magnific-popup.min.js"></script>
 <script src="js/plugins.js"></script>
 <script src="js/gijgo.min.js"></script>
 <!--contact js-->
 <script src="js/contact.js"></script>
 <script src="js/jquery.ajaxchimp.min.js"></script>
 <script src="js/jquery.form.js"></script>
 <script src="js/jquery.validate.min.js"></script>
 <script src="js/mail-script.js"></script>
 <script src="js/main.js"></script>
</body>
</html>